...
$(document).ready(function(){
  $('#buytxt').change(function(){
    var bt = $('#buytxt').val().replace('.','').replace('.','').replace('.','').replace(" đồng",'');
    var change_bt = numeral(bt).format('0,0');
    if(change_bt){
      $('#buytxt').val(change_bt.replace(/,/g,'.')+' đồng');
    }
  });
});
function ChangePrice(){
 var rate = $('#select_rate').val();
 var pprice = $('#pre_pay').val().replace(/[ đồng.]/g,''); 
 var price = $('#price').val().replace(/[ đồng.]/g,'');
 if(rate!='')
 {
  var tam=numeral(price*rate).format('0,0');
  $('#pre_pay').val(tam.replace(/,/g,'.')+" đồng");
  $('#amount_slow').val((numeral(price-(price*rate)).format('0,0')).replace(/,/g,'.')+" đồng");
}
...
var mon = $('#lead_month').val();
var asl = numeral((price-price*rate)).format('0,0');
if(mon){
  //console.log(mon,asl.replace(/,/g,''),interest_rate,price,pprice);
  // $('#amount_slow').val(asl.replace(/,/g,'.')+" đồng");
  var sml = numeral((((interest_rate*asl.replace(/,/g,''))+parseInt(asl.replace(/,/g,'')/mon)+parseInt(11000)))).format('0,0');
  $('#slow_month').val(sml.replace(/,/g,'.')+" đồng");
}
}
function ChangeSalary_avg(){
 var salary_avg = $('#salary_avg').val().replace(/[ đồng.]/g,'');
 var a = numeral(salary_avg).format('0,0');
 if(a!=""){
  $("#salary_avg").val(a.replace(/,/g,'.')+" đồng");
}
}
function ChangePP(){
  var price = $('#price').val().replace(/[ đồng.]/g,'');
  ...
  var mon = $('#lead_month').val();
  var asl = numeral(price-pprice).format('0,0');
  $('#amount_slow').val(asl.replace(/,/g,'.')+" đồng");
  var sml = numeral((((rate*asl.replace(/,/g,''))+parseInt(asl.replace(/,/g,'')/mon)+parseInt(11000)))).format('0,0');
  $('#slow_month').val(sml.replace(/,/g,'.')+" đồng");
  var a = numeral(pprice).format('0,0');
  if(a!=""){
    $("#pre_pay").val(a.replace(/,/g,'.')+" đồng");
    $("#select_rate").prop('disabled',true);
    $("#select_rate").val("");
  }
  if(pprice=='')
  {
...
$(document).ready(function(){
  $("#select_rate").change(function(){
    var a = $("#select_rate").val();
    if(a==""){
      $("#pre_pay").prop('readonly',false);
      $('#pre_pay-error').val("");
    }
    else{
      $('#pre_pay-error').remove();
      $('#pre_pay').removeClass('error');
      var price = $('#price').val().replace(/[ đồng.]/g,'');
      var pprice = price*a;
      $("#pre_pay").prop('readonly',true);
      $("#pre_pay").val((numeral(pprice).format('0,0')).replace(/,/g,'.')+" đồng");
      var price = $('#price').val().replace(/[ đồng.]/g,'');
      var rate = $('#interest_rate').val();
      var mon = $('#lead_month').val();
      var asl = numeral(price-pprice).format('0,0');
  ...
  $('#salary_info').change(function(){
    var sinfo = $('#salary_info').val().replace(/[  đồng,.]/g,'');
    if(sinfo){
      $('#salary_info').val(numeral(sinfo).format('0,0').replace(/[,]/g,'.')+" đồng");
    }
  });
  // var savg = $('#salary_avg').val().replace(/[ đồng,.]/g,'');
  // var pp = $("#pre_pay").val().replace(/[ đồng.]/g,'');
  // var p = $("#price").val().replace(/[ đồng.]/g,'');
  // var am = $('#amount_slow').val().replace(/,/g,'.');
  // var slm = $('#slow_month').val().replace(/,/g,'.');
  // var buyuse = $('#buy_use').val().replace(/[ đồng,.]/g,'');
  // var sinfo = $('#salary_info').val().replace(/,/g,'.');
  // var salary = $('#salary').val().replace(/,/g,'.');
  // var buy =numeral(buytxt.replace(/[ đồng]/g,'')).format('0,0');
  // var bs = numeral(buyuse).format('0,0');
  // var a = numeral(pp).format('0,0');
  // var b = numeral(p).format('0,0');
  // if(a){
  //   $("#pre_pay").val(a.replace(/[,]/g,'.')+" đồng");
  // }

  // if(b){
  //   $("#price").val(b.replace(/[,]/g,'.')+" đồng");
  // }
  // if(am){
  //   $('#amount_slow').val(am.replace(' đồng','')+" đồng");
  // }
  // if(slm){
  //   $('#slow_month').val(slm.replace(' đồng','')+" đồng");
  // }
  // if(sinfo){
  //   $('#salary_info').val(sinfo.replace(' đồng','')+" đồng");
  // }
  // if(salary){
  //   $('#salary').val(salary.replace(' đồng','')+" đồng");
  // }
  // if(bs){
  //   $('#buy_use').val(bs.replace(/,/g,'.')+" đồng");
  // }
  // $('#buytxt').val(formatNumber($('#buytxt').val()).replace(/,/g,'.'));
  // var buytxt = $('#buytxt').val().replace(/[ đồng.]/g,'');
  // if(buytxt){
  //   $('#buytxt').val(buy.replace(/,/g,'.')+" đồng");
  // }
